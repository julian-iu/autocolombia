...
if ($placa != "" && $mes != "MES" && $ano != "") { // buscar ano y mes
    ...
    if ($cantidad == 0) {
        ...
        $sqlinformacion = "SELECT * FROM parqueo, vehiculo, celda WHERE p_vehiculoFK = v_id AND p_celdaFK = c_id  AND MONTH(p_fechasalida) = " . $mes . " AND YEAR(p_fechasalida) = " . $ano . " AND p_estado = '2' AND  p_vehiculoFK  = " . $v_id . " AND p_pago = '0' ";
        ...
        foreach ($pdo->query($sqlinformacion) as $info) {
            ...
            $sql = "INSERT INTO pago (pg_parqueoFK, pg_monto, pg_fechapago) VALUES (?, ?, ?);";
            $ejecutar = $pdo->prepare($sql);
            $ejecutar->execute(array($info['p_id'], $suma, $fechasistema));

            $sqlupdate = "UPDATE parqueo SET p_pago = ? WHERE p_id  = ?;";
            $ejecutarup = $pdo->prepare($sqlupdate);
            $ejecutarup->execute(array('1', $info['p_id']));
