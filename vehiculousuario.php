...
$eje = $pdo->prepare($buscarusuario);
$eje->execute(array($v_usuarioFK));
$datousuveh = $eje->fetch(PDO::FETCH_ASSOC);
$u_id = $datousuveh['u_id'];
$nombrevehiculo = $datousuveh['u_nombre'];
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($accion == 'agregar') {
        ...
        $id = $_POST['id'];
        $sqlcantidad = "SELECT COUNT(*) FROM lavado WHERE l_vehiculoFK = " . $id . "";
        $query = $pdo->query($sqlcantidad);
        $cantidad = $query->fetchColumn();
        if ($cantidad != 0) {
            echo '<script type="text/javascript">';
            echo 'setTimeout(function () { swal("Información!", "Información en Lavado","error");';
            echo '}, 1000);</script>';
        } else {
...
<body>
            ...
            <div class="modal-content ">
                ...
                <div class="modal-body">
                    <form ROLE="FORM" METHOD="POST" ACTION="">
                        ...
                        <input type="hidden" class="form-control" name="v_usuarioFK" class="form-control" value="<?php echo $u_id ? $u_id : '' ?>" required>

                        <span>* Ingrese Información</span><br><br>
                        <div class="input-group input-group-sm mb-3">
                            <span class="input-group-text"><i class="fa-regular fa-address-card"></i></span>
                            <input type="text" class="form-control" name="v_placa" class="form-control" placeholder="No. PLACA" value="<?php echo htmlspecialchars(isset($_SESSION['v_placa']) ? $_SESSION['v_placa'] : '');?>" required>
                        </div>
                        <div class="input-group input-group-sm mb-3">
                            <span class="input-group-text"><i class="fa-brands fa-bandcamp"></i></span>
                            <input type="text" class="form-control" name="v_marca" placeholder="Marca del Vehiculo" value="<?php echo htmlspecialchars(isset($_SESSION['v_marca']) ? $_SESSION['v_marca'] : '');?>">
                        </div>
                        <div class="input-group input-group-sm mb-3">
                            <span class="input-group-text"><i class="fa-solid fa-font"></i></span>
                            <input type="text" class="form-control" name="v_modelo" placeholder="Modelo del Vehiculo" value="<?php echo htmlspecialchars(isset($_SESSION['v_modelo']) ? $_SESSION['v_modelo'] : '');?>" required>
                        </div>
                        <div class="input-group input-group-sm mb-3">
                            <span class="input-group-text"><i class="fa-solid fa-palette"></i></span>
                            <input type="text" class="form-control" name="v_color" placeholder="Color del Vehiculo" value="<?php echo htmlspecialchars(isset($_SESSION['v_color']) ? $_SESSION['v_color'] : '');?>" required>
                        </div>
                        <center> <button type="submit" class="btn btn-primary btn-sm">Guardar &nbsp;&nbsp;&nbsp;&nbsp;<i class="fa-regular fa-floppy-disk"></i></button></center>
                    </form>
                </div>
    ...
    <div class="card">
        <div class="card-header text-white">
            <strong>TABLA DE <?php echo $titulopagina; ?> DE  <?php echo $nombrevehiculo;?> &nbsp;&nbsp;&nbsp; 
        ...
        <div class="card-body">
            <table id="datatablesSimple" class="display table table-bordered table-hover">
                ...
                <tbody>
                    <?php
                    $informacion = 'SELECT * FROM vehiculo WHERE v_usuarioFK = ' . $u_id . ' ORDER BY v_placa ASC;';
                    $contador = 1;
